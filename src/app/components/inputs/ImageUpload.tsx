...
const ImageUpload = ({ onFileChange }: any) => {
            ...
            reader.onloadend = () => {
                ...
                fetch('/api/files', {
                    ...
                    .then((data) => {
                        ...
                        onFileChange(data.data?.id);
                        setPreview(data?.data?.fullUrl as string | null);
                    })
                    .catch((error) => {
                        alert("ERROR")
                    })
    ...
    return (
        <>
            <div
                id="image-preview"
                className={`// eslint-disable-next-line prettier/prettier mb-4 cursor-pointer 
                items-center rounded-lg border-2
                ${preview
                        ? 'border-transparent'
                        : 'border-dashed border-gray-400'
                    } bg-gray-100 p-6 text-center`}
                onClick={handleClick}
            >
                <input
                    id="upload"
